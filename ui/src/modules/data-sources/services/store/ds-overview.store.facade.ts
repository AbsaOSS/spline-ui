...
import { DsOverviewStore, DsOverviewStoreActions, DsOverviewStoreSelectors } from '../../store'
...
export class DsOverviewStoreFacade {
    ...
    init(dataSourceId: string): void {
        this.store.dispatch(
            new DsOverviewStoreActions.InitRequest({
                dataSourceId
            })
        )
    }
    ...
    unInit(): void {
        this.store.dispatch(
            new DsOverviewStoreActions.ResetState()
        )
    }
}
